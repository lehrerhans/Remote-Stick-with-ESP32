...
// Fn (top Button): Switch between mouse or cursor mode
// M  (RH upper Button): switch through all screen-button-menues. (1-1, 2-2, 3-3-3, 4-4-4)
// X lower button: short press opens F1-Menue(Quick Menu), long press is ESC
...
void loop() {
    if (btn_left.pressed > 0) {
      ...
      if (btn_left.pressed == 1) { //Short press
        Keyboard.releaseAll();
    ...
    if (btn_right.pressed > 0) {
      ...
      if (btn_right.pressed == 1) { //Short press
        Keyboard.releaseAll();
    ...
    if (btn_down.pressed > 0) {
      ...
      if (btn_down.pressed == 1) { //Short press
        Keyboard.releaseAll();
    ...
    if (btn_up.pressed > 0) {
      Serial.printf("up %u \n",btn_up.pressed);
      if (btn_up.pressed == 1) { //Short press
        Keyboard.releaseAll();
